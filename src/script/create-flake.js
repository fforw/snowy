...
function getMaxValue(canvas, size)
{
    var data = canvas.getContext("2d").getImageData(0, 0, size, size).data;
...
function transform(canvas, max, size)
{
...
module.exports = function (opts)
{
    ...
    var i, j, w, flakeSize = opts.size;
    var size = (flakeSize + opts.border + opts.border) | 0;
    ...
    var colIdx = 255 - colorStep * numLineWidths;

    var startAngle = Math.random() * FLAKE_STEP;

    for (j = 0; j < numLineWidths; j++)
    {
        w = opts.lineWidth[j];
        ctx.lineWidth = w;
        var color = opts.palette[colIdx];
        //console.debug("Color = %s, width = %n", color, w);
        ctx.strokeStyle = color;

        for (var a = startAngle; a < TAU; a += FLAKE_STEP)
        {
            var x = cx;
            var y = cy;
            var dx = Math.cos(a) * flakeSize;
            var dy = Math.sin(a) * flakeSize;
            ...
            for (i = 0; i < spokes; i++)
            {
                ...
                ctx.beginPath();
                ctx.moveTo(x, y);
                ctx.lineTo(x + Math.cos(a - FLAKE_STEP / 2) * len, y + Math.sin(a - FLAKE_STEP / 2) * len);
                ctx.moveTo(x, y);
                ctx.lineTo(x + Math.cos(a + FLAKE_STEP / 2) * len, y + Math.sin(a + FLAKE_STEP / 2) * len);
                ctx.stroke();
            }
